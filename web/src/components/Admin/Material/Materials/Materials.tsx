...
import { QUERY } from 'src/components/Admin/Material/MaterialsCell'
        ...
        <tbody>
          {materials.map((material) => (
            <tr key={material.id}>
              ...
              <td>{truncate(material.type)}</td>
              <td>{truncate(material.quantity)}</td>
              <td>{truncate(material.unit)}</td>
              <td>{formatEnum(material.status)}</td>
              <td>{truncate(material.price)}</td>
              <td>{timeTag(material.expected_delivery_date)}</td>
              <td>{timeTag(material.delivery_date)}</td>
              <td>{truncate(material.taskId)}</td>
              <td>{timeTag(material.createdAt)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.adminMaterial({ id: material.id })}
